...
func TestAVSourceType(t *testing.T) {
	p := NewSentinelAVParser()
	if p.SourceType() != "sentinel_av" {
		t.Errorf("SourceType() = %q, want %q", p.SourceType(), "sentinel_av")
	}
}
...
func TestAVScanResultMalicious(t *testing.T) {
	...
	if event.AV.Scan.Engine != "sentinel-yara" {
		t.Errorf("av.scan.engine = %q, want sentinel-yara", event.AV.Scan.Engine)
	}
